...
const Post = ({ post }) => {
		...
		mutationFn: async () => {
		  try {
			const res = await fetch(`/api/posts/comment/${post._id}`, {
			  ...
			  body: JSON.stringify({text:comment}), // Trim unnecessary spaces
			});
			const data = await res.json();
			if (!res.ok) {
			  throw new Error(data.error || "Something went wrong");
			}
			return data; // `updatedComments` expected here
		  } catch (error) {
			throw new Error(error.message || "An error occurred while posting the comment");
		  }
		},
	...
	const formattedDate = formatPostDate(post.user.createdAt);
